...
5. Script Processors
6. Tree construction. Order of script processingh.Layout and Painting
...
You enter "github.com" (that is called "a domain") but actually, the server which hosts the source code of a website, is identified via IP (= Internet Protocol) addresses. The browser sends a "request" (see step 2) to the server with the IP address you entered (indirectly - you of course entered "github.com").
...
In reality, you also often enter "github.com/jasham" or anything like that. "github.com" is the domain, "/jasham" is the so-called path. Together, they make up the "URL" ("Uniform Resource Locator").

In addition, you can visit most websites via "www.github.com" or just "github.com". Technically, "www" is a subdomain but most websites simply redirect traffic to "www" to the main page.

<hr>

An IP address typically looks like this: 20.207.73.82 (though there also is a more "modern" form called IPv6 - but let's ignore that for now). You can learn more about IP addresses on Wikipedia.

How is the domain "github.com" translated to its IP address?
...
When you enter "github.com", the browser therefore first fetches the IP address from such a DNS server.
<br>
...
Because HTTP is used, a full URL actually looks like this: http://github.com. The browser auto-completes it for you.

And there also is HTTPS - it's like HTTP but encrypted. Most modern pages (including github.com) use that instead of HTTP. A full URL then becomes: https://github.com.
...
here `<h1>` and `<p>` are so called "HTML tags"
...
```
<link crossorigin="anonymous" media="all" rel="stylesheet" href="https://github.githubassets.com/assets/dashboard-74d536a2af1a.css" />
```
...
These are the four steps that are always involved when you enter a page address like github.com and you thereafter see the website content in your browser.
...
Thus far, we discussed websites. I.e. the case where you enter a URL (e.g. "github.com/jasham") into the browser and you get a website in return.
...
You can't request a PDF from "github.com" for example. You could send such a request but you wouldn't get back a PDF - simply because we don't support this kind of requested data for this specific URL.
